        ...
        try:
            if self.action in ["restart", "shutdown"]:
                ...
                data = {
                    ...
                    "reason": self.reason, 
                    "executor": interaction.user.name,
                    "username": self.username  # Added username for Roblox script
                }
...
class BanListView(View):
    ...
    def get_embed(self):
        ...
        for i in range(start_idx, end_idx):
            ...
            userid = ban.get('userid', 'Unknown')
            username = ban.get('username', 'Unknown')
            display_name = ban.get('display_name', username)  # Use display_name or username
            reason = ban.get('reason', 'No reason')
            executor = ban.get('executor', 'Unknown')
    ...
    async def refresh_list(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Enrich ban data with Roblox information
            enriched_bans = []
            for ban in bans:
                userid = ban.get('userid', 'Unknown')
                username = ban.get('username', 'Unknown')

                # If username is generic, try to get data from Roblox
                if username in ["Unknown", "1"] and userid != "Unknown":
                    ...
                    # Check if Display Name exists in data
            ...
            # Automatically save updated banlist
...
class UnbanView(View):
    ...
    async def confirm_unban(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            # Send the correct command based on ban type
            if self.ban_type == "pc":
...
class UnPCBanView(View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            # Use unpcban command - this should match exactly what the Roblox script expects
            unpcban_data = {
                "command": "unpcban",  # This must be "unpcban" exactly
                "userid": self.userid,
                "executor": interaction.user.name
            }
...
def check_api():
    try:
        # Check API availability via player count endpoint
...
async def ban_command(interaction: discord.Interaction, userid: str, reason: str, duration: int = -1, ban_type: str = "normal"):
    ...
    embed = ModerationEmbed(
        title=f"⚠️ Confirm {ban_type_display}",
...
async def banlist_command(interaction: discord.Interaction):
    ...
    try:
        ...
        # AUTOMATICALLY save banlist to server files
        saved_filepath = save_banlist_to_disk(enriched_bans)
...
async def unban_command(interaction: discord.Interaction, userid: str, ban_type: str = "auto"):
    ...
    try:
        ...
        embed = ModerationEmbed(
            title=f"⚠️ Confirm {'PC ' if detected_ban_type == 'pc' else ''}Unban",
...
async def players_command(interaction: discord.Interaction):
    ...
    try:
        try:
            response = requests.get(f"{API_URL}/get_players_detailed", timeout=10)
            if response.status_code == 200:
                players = response.json()
            else:
                response = requests.get(f"{API_URL}/get_players", timeout=10)
                players_data = response.json()
                players = [{"username": f"Player {i+1}", "userid": "Unknown", "playtime": "0"} 
...
async def unpcban_command(interaction: discord.Interaction, userid: str):
    ...
    # Use UnPCBanView instead of UnbanView for PC bans
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value="""`/ban [id] [reason] [duration] [type]` - Ban player (duration in days, -1 for permanent, type: normal/pc)
`/kick [id] [reason]` - Kick player
`/unban [id] [type]` - Unban player (type: auto/normal/pc)
`/warn [id] [reason]` - Warn player
`/mute [id] [duration] [reason]` - Mute player
`/pcban [id] [reason]` - PC BanAsync player (device-based, permanent)
`/unpcban [id]` - Remove PC ban""",
    ...
    embed.add_field(
        ...
        value="""`/banlist` - View all bans (automatically saves to server files)
...
`/broadcast [msg]` - Broadcast""",
    ...
    embed.set_footer(text=f"Total commands: 19 • Version 3.2 • Auto-save Banlist: ✅ Server Files")
    await interaction.response.send_message(embed=embed)
...
🔨 **Moderation:**
`/ban`, `/kick`, `/unban`, `/warn`, `/mute`, `/pcban`, `/unpcban`

📋 **Players & Lists:**
`/banlist` - Automatically saves to server
`/players`, `/find`, `/lookup`, `/stats`
...
`/restart`, `/shutdown`, `/announce`, `/broadcast`"""
    ...
    embed.set_footer(text="Use /help for detailed information • Banlist: Auto-saves to server files")
    await interaction.response.send_message(embed=embed)
...
def run():
    if TOKEN:
        ...
        logger.info(f"Total commands loaded: 19")
